...
export function csrfProtection(config) {
  const { doubleCsrfProtection } = doubleCsrf({
    ...
    // req.session.id is unusable here: with saveUninitialized false an
    ...
    // cookie. Binding to req.session.userId instead — the only field this
    // app ever writes to the session — needs no extra session write, is
    // stable for the whole anonymous phase, and still changes (invalidating
    // old tokens) exactly when §6.1's session regenerate on login takes effect.
